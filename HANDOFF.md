...
| **Last session focus** | Pipeline Logging & Observability Enhancement: Added structured logging across `src/models/baselines.py` (majority class distribution, OLS R² fit), `src/models/train.py` (null-drop row counts, positive class rate, wall-clock timing, top-5 LR coefficients, top-5 RF feature importances), and `src/models/evaluation.py` (per-symbol summary & per-regime metrics). Configured `logging.basicConfig()` in notebooks 02 and 03. Verified all 31 pytest unit tests pass cleanly in 70s. |
